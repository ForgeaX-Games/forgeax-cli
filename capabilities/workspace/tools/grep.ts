...
export default {
  ...
  guidance: "**grep**: Preferred over shell grep/rg. Use output_mode='files_with_matches' for file-only results. Use glob for filename patterns.",
  input_schema: {
    type: "object",
    properties: {
      pattern: {
      ...
      path: {
        type: "string",
        description: "Directory or file to search in. Relative paths resolve from CURRENT_DIR. Defaults to CURRENT_DIR.",
      },
      case_insensitive: {
        type: "boolean",
      ...
      head_limit: {
        type: "integer",
        description: "Maximum number of result lines to return. Default: 500",
      },
      offset: {
        type: "integer",
        description: "Skip the first N result lines (for pagination). Default: 0",
      },
      multiline: {
        type: "boolean",
