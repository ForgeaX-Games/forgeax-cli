...
export default {
  ...
  // ^ Note: same-turn race (multiple mutators on same path) is mitigated by the
  //   GENERAL_GUIDELINES "split dependent calls into separate turns" rule above.
  input_schema: {
    type: "object",
    properties: {
      path: {
  ...
  async execute(args, ctx): Promise<ToolOutput> {
    ...
    try {
      await ctx.fs.writeText(absPath, updated);
    } catch (e: any) {
