...
export default {
  ...
  description:
    ...
    "Can write to team paths and absolute paths outside the agenteam root.",
  input_schema: {
    type: "object",
    properties: {
      path: { type: "string", description: "File path. Relative paths resolve from CURRENT_DIR. May write team paths and absolute paths outside the agenteam root." },
  ...
  async execute(args, ctx): Promise<ToolOutput> {
    ...
    if (existing?.isFile) {
      ...
      } catch { /* file unreadable, proceed with write */ }
    }
