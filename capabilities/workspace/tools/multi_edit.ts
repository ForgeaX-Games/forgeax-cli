...
import { findActualString, applyEditToFile, preserveQuoteStyle, stripTrailingWhitespace } from "../lib/edit-utils.js";
...
export default {
  ...
  guidance: "**multi_edit**: Prefer over multiple edit_file calls for same-file changes — atomic and context-efficient. On first failure, re-read and copy old_string exactly; if the patch is too large or repeated failures occur, use write_file.",
  input_schema: {
    type: "object",
    properties: {
      path: {
        type: "string",
        description: "File path (absolute or relative to CURRENT_DIR)",
      },
      edits: {
        type: "array",
        items: {
          ...
          properties: {
            old_string: {
              type: "string",
              description: "The exact text to replace at this step (must match current content exactly)",
            },
            new_string: {
              type: "string",
        ...
        minItems: 1,
        description: "Array of edit operations to apply sequentially. Each edit runs against the result of previous edits.",
      },
    },
    required: ["path", "edits"],
  },
  async execute(args, ctx): Promise<ToolOutput> {
    ...
    let content: string;
    try {
      content = await ctx.fs.readText(absPath);
    } catch {
    ...
    for (let i = 0; i < edits.length; i++) {
      ...
      if (!actualOldStr) {
        return `Edit ${i + 1}/${edits.length} failed: old_string not found in ${absPath}. Make sure the text matches exactly, including whitespace and indentation. Hint: Re-read the file with read_file and copy the exact text. No edits applied.`;
      }
    ...
    try {
      await ctx.fs.writeText(absPath, content);
    } catch (e: any) {
