...
import { fileToContentPart, isBinaryBuffer } from "#src/core/types.js";
import { recordFileRead } from "../lib/file-state.js";

const MAX_CHARS = 256_000;
const MAX_LINE_LENGTH = 2000;
/** Hard ceiling to prevent reading multi-GB files. Normal truncation is handled by MAX_CHARS. */
const HARD_SIZE_LIMIT = 512 * 1024 * 1024; // 512 MB
/**
 * For text files without offset/limit, only read enough bytes to fill MAX_CHARS.
 * Assumes ~1 byte per char (UTF-8 ASCII-dominant). Generous buffer for multi-byte.
 */
...
export default {
  ...
  async execute(args, ctx): Promise<ToolOutput> {
    ...
    // Unified path — ctx.fs routes host vs container automatically.
    ...
    // For media / binary files we need enough to detect type (isBinaryBuffer uses first 8KB).
    // For large text without range, read only enough to fill MAX_CHARS.
