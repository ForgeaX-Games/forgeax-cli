...
// 恢复(而非 delete)preload 设的沙盒目录,保住整进程的密闭性护栏(test/preload.ts)。
const preloadConfigDir = process.env.FORGEAX_CONFIG_DIR;
afterEach(() => {
  if (preloadConfigDir === undefined) delete process.env.FORGEAX_CONFIG_DIR;
  else process.env.FORGEAX_CONFIG_DIR = preloadConfigDir;
