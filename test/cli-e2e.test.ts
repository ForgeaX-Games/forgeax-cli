/**
 * CLI e2e —— 驱动**真实 forgeax-cli 二进制**(子进程),指向本地 mock Anthropic
        ...
        payload =
          forkCalls === 1 && memDir
            ? toolUseTurn('write_file', {
                ...
                content: '---\nname: E2E Pref\ndescription: set in e2e\ntype: user\n---\nuser validated forgeax-cli via e2e\n',
              })
...
describe('CLI e2e — real binary against mock Anthropic', () => {
  ...
  test('auto-memory: end-of-turn extract (cache-safe fork) writes a real .md + MEMORY.md index', async () => {
    ...
    try {
      ...
      expect(readFileSync(join(memDir, mem), 'utf8')).toContain('validated forgeax-cli via e2e');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  }, 30000);
  ...
  test('--help / --version (form factor, no network)', async () => {
    ...
    try {
      ...
      expect(v.out).toContain(`forgeax-cli ${FORGEAX_CORE_VERSION}`);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  }, 30000);
  ...
  test('--demo: built-in echo provider, no key needed', async () => {
    ...
    try {
      ...
      const code = await proc.exited;
      expect(code).toBe(0);
      expect(out).toContain('forgeax-cli(demo)');
