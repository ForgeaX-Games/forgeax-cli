...
describe('AskUserQuestion via dispatchTools', () => {
  ...
  test('invalid input fails before hooks, permissions, and host call', async () => {
    let hookRan = false;
    let permissionRan = false;
    const { fn, seen } = stubAskQuestion(() => []);
    const tool = askUserQuestionTool();
    const [result] = await dispatchTools(
      [{ id: 'tu_invalid', name: 'AskUserQuestion', input: { questions: [] } }],
      {
        tools: [tool],
        toolContext: { askQuestion: fn },
        signal: new AbortController().signal,
        trusted: true,
        isBlocked: () => { hookRan = true; return false; },
        preToolPermission: () => { permissionRan = true; return undefined; },
      },
    );
    expect(result).toMatchObject({ isError: true, errorCategory: 'validation' });
