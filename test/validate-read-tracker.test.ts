 ...
 * 覆盖 type/required/properties/additionalProperties/items 长度/enum/pattern/oneOf/anyOf + permissive 兜底。
 */
...
describe('validateAgainstSchema — type', () => {
  ...
  test('非 JSON 值不能伪装成 object', () => {
    expect(validateAgainstSchema(() => undefined, { type: 'object' }).ok).toBe(false);
    expect(validateAgainstSchema(Symbol('x'), { type: 'object' }).ok).toBe(false);
    expect(validateAgainstSchema(1n, { type: 'object' }).ok).toBe(false);
  });
});
...
describe('validateAgainstSchema — closed objects / array length', () => {
  test('additionalProperties:false 拒绝顶层与嵌套多余字段并返回精确 path', () => {
    const schema = {
      type: 'object',
      properties: {
        known: {
    ...
    expect(validateAgainstSchema({ known: { ok: true } }, schema)).toEqual({ ok: true });
    const top = validateAgainstSchema({ known: { ok: true }, extra: 1 }, schema);
    expect(top).toMatchObject({ ok: false, path: '$.extra' });
    const nested = validateAgainstSchema({ known: { ok: true, extra: 1 } }, schema);
    expect(nested).toMatchObject({ ok: false, path: '$.known.extra' });
    expect(validateAgainstSchema({ constructor: 'unexpected' }, schema)).toMatchObject({
      ok: false,
      path: '$.constructor',
    });
    expect(validateAgainstSchema({ toString: 'unexpected' }, schema)).toMatchObject({
      ok: false,
      path: '$.toString',
    });
  });

  test('additionalProperties 缺省或 true 继续放行', () => {
    expect(validateAgainstSchema({ extra: 1 }, { type: 'object', properties: {} })).toEqual({ ok: true });
    expect(validateAgainstSchema({ extra: 1 }, { type: 'object', properties: {}, additionalProperties: true })).toEqual({ ok: true });
  });

  test('minItems/maxItems 约束数组长度', () => {
    const schema = { type: 'array', minItems: 1, maxItems: 2 };
    expect(validateAgainstSchema([1], schema)).toEqual({ ok: true });
    expect(validateAgainstSchema([], schema)).toMatchObject({ ok: false, path: '$' });
    expect(validateAgainstSchema([1, 2, 3], schema)).toMatchObject({ ok: false, path: '$' });
  });
});
