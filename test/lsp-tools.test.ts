...
describe('jsonrpc framing', () => {
  ...
  // ── 回归:区分「spawn 失败(命令不存在)」与「进程退出」的错误文案 ──────────
  //
  // 修复前:spawn ENOENT 被塞成 onClose({code:null,signal:null}),报出误导性的
  // 「language server exited (code=null, signal=null)」,让人以为 server 崩了。
  // 修复后:携带 error(ENOENT)时应给出可操作的「not found — install ...」提示。

  test('closeErrorMessage:ENOENT 给出可操作的 not-found 提示(非 exited)', () => {
    const enoent: NodeJS.ErrnoException = Object.assign(
      new Error('spawn typescript-language-server ENOENT'),
      { code: 'ENOENT', path: 'typescript-language-server' },
    );
    const msg = closeErrorMessage({ code: null, signal: null, error: enoent }, 1);
    expect(msg).toMatch(/not found/);
    expect(msg).toMatch(/install/);
    expect(msg).toContain('typescript-language-server');
    // 关键:不再误报为「exited (code=null」。
    expect(msg).not.toMatch(/exited/);
  });

  test('closeErrorMessage:真正的进程退出保留 code/signal', () => {
    const msg = closeErrorMessage({ code: 1, signal: null }, 0);
    expect(msg).toMatch(/language server exited \(code=1, signal=null\)/);
    expect(msg).not.toMatch(/not found/);
  });

  test('JsonRpcClient:onClose 带 ENOENT error → reject 未决请求为 not-found', async () => {
    const closeBox: { fn: ((info: { code: number | null; signal: string | null; error?: NodeJS.ErrnoException }) => void) | null } = { fn: null };
    const transport: RpcTransport = {
      write() {},
      onData() {},
      close() {},
    ...
    // 模拟 defaultSpawner 在命令不存在时上报的 close 信息。
    const enoent: NodeJS.ErrnoException = Object.assign(
      new Error('spawn typescript-language-server ENOENT'),
      { code: 'ENOENT', path: 'typescript-language-server' },
    );
    closeBox.fn?.({ code: null, signal: null, error: enoent });
    await expect(pending).rejects.toThrow(/not found[\s\S]*install/);
  });
});
