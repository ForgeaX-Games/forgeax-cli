 ...
 *
 * 两特性均**加性 + 默认关**:
 *   - 不注入 deps.background ⇒ schema 不暴露 run_in_background;直接调用仍同步跑。
 *   - isolation 始终在 schema 暴露,未设时 worktree 接缝完全不触碰。
...
describe('makeTaskTool — run_in_background', () => {
  test('schema 仅在注入 background 时暴露后台能力;isolation 始终暴露', () => {
    const withoutBackground = makeTaskTool({
      provider: scripted([[asstText('sync')]]), model: 'm', resolveTools: () => [echo],
    });
    const withBackground = makeTaskTool({
      provider: scripted([[asstText('background')]]), model: 'm', resolveTools: () => [echo],
      background: new BackgroundTasks<SubagentResult>(),
    });
    const propertiesOf = (task: ReturnType<typeof makeTaskTool>) =>
      (task.inputJSONSchema as { properties?: Record<string, unknown> }).properties ?? {};

    expect(propertiesOf(withoutBackground).run_in_background).toBeUndefined();
    expect(propertiesOf(withBackground).run_in_background).toBeDefined();
    expect(propertiesOf(withoutBackground).isolation).toBeDefined();
    expect(propertiesOf(withBackground).isolation).toBeDefined();
  });
  ...
  test('未注入 background ⇒ 直接传 run_in_background 仍照常同步跑', async () => {
