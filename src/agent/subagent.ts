...
export interface TaskInput {
  ...
  /**
   * 可选(P2):后台跑子 agent。仅在 `TaskToolDeps.background` 已注入时向模型暴露。
   * 为 true 时不 await 子 loop —— 把子 promise 登记进 `background`,立即返回占位结果
   * (`turns:0/toolCalls:0`,text 标注 `id=...`)。子跑完经 `onBackgroundDone` 回调。
   * 未注入 `background` 时直接调用方传入本标记仍会照常同步跑。
   */
...
export interface TaskToolDeps {
  ...
  bus?: EventBusAPI;
  /**
   * 可选(P2):后台任务登记处。注入后 Task schema 才暴露 `run_in_background`;
   * `input.run_in_background === true` 时不 await 子 loop,而是登记并立即返回占位结果。
   * 缺省时 schema 不声明该能力;直接调用方即使传入标记也照常同步跑。
   */
        ...
        ...(deps.background
          ? {
              run_in_background: {
                type: 'boolean',
                description:
                  'Run this agent as a background task (non-blocking): the call returns a placeholder immediately and the result arrives later as a task notification. Use for long-running work that should not block the main thread.',
              },
            }
          : {}),
        isolation: {
          ...
          enum: ['worktree'],
          description:
            'Set to "worktree" to run the agent in an isolated temporary git worktree (auto-removed when the agent finishes; falls back to cwd outside a git repo).',
        },
      },
      required: ['prompt'],
    },
