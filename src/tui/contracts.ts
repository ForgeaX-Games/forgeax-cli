...
//   user 条目可带 msgId:回退点的文件快照锚点(submit 时由 TUI 预生成、driver 据此拍快照)。
...
export interface StatusState {
  ...
  elapsedMs?: number;
  busy: boolean;
  /** 运行中的后台 shell 数(>0 时正文区 ThinkingIndicator 常驻静态计数标签;T4.5 感知)。 */
  bgShells?: number;
  /** 运行中的后台子 agent 数(与 bgShells 同渲染一枚标签:纯 shell/纯 agent/混合分文案)。 */
  bgAgents?: number;
...
export interface AgentDriver {
  ...
  /** 运行中的后台任务计数(正文区常驻计数标签用;shell 与子 agent 分开报,渲染层定文案)。 */
  backgroundTaskCounts(): { shells: number; agents: number };
   ...
   *  preferredMsgId 供 TUI 先把 user 条目上屏、再拍快照:避免大工作区同步扫描阻塞首屏。 */
