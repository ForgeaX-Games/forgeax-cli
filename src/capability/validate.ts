 ...
 *   - 只覆盖被工具实际用到的子集:type / required / properties(递归) /
 *     additionalProperties:false / enum / pattern / items / minItems / maxItems /
 *     oneOf / anyOf;未识别的关键字一律忽略(不当作错误)。
...
function actualType(value: unknown): JsonSchemaType | 'null' | 'undefined' | 'function' | 'symbol' | 'bigint' {
  ...
  // JSON 不含 function / symbol / bigint；保留实际类型以确保不匹配 object。
  return t as 'function' | 'symbol' | 'bigint';
}
    ...
    if (schema.additionalProperties === false) {
      for (const key of Object.keys(obj)) {
        if (!props || !Object.hasOwn(props, key)) {
          return fail(childPath(path, key), `unexpected property "${key}"`);
        }
      }
    }
  }

  // array:长度约束 + items 递归(支持单一 items schema;tuple 形式也按位递归)。
  if (Array.isArray(value)) {
    if (typeof schema.minItems === 'number' && value.length < schema.minItems) {
      return fail(path, `expected at least ${schema.minItems} item(s), got ${value.length}`);
    }
    if (typeof schema.maxItems === 'number' && value.length > schema.maxItems) {
      return fail(path, `expected at most ${schema.maxItems} item(s), got ${value.length}`);
    }
  }
