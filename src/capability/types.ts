...
export interface AgentTool<Input = unknown, Output = unknown> {
  ...
  readonly inputSchema?: {
    parse(x: unknown): Input;
    safeParse(x: unknown): { success: true; data: Input } | { success: false; error?: unknown };
  };
...
export interface PluginContext {
  agentId?: string;
  config?: Readonly<Record<string, unknown>>;
  /** loop 同一 EventBus(assemble 启动 plugin 时传入)。缺省 = host 不接事件,
   *  plugin 应优雅降级(不订阅),不得报错。 */
  bus?: EventBusAPI;
  [key: string]: unknown;
}
