 ...
 * layered-memory + soul-pack 留 @forgeax/orchestrator 启动注入，core/src 零 soul 代码) +
...
export interface RememberOutput {
  path: string;
  /** 新记忆在 `MEMORY.md` 里的规范索引行(manifest 格式)。常驻索引按会话记忆化
   *  (缓存前缀稳定,见 memory/slot.ts),中途新写的记忆靠这行 append-only 进对话
   *  历史让模型可见(对齐 cc:变化走追加式提醒,不回改已缓存内容)。 */
  indexLine?: string;
}
