...
export function makeRememberTool(deps: MemoryToolDeps): AgentTool<RememberInput, RememberOutput> {
  ...
  return buildTool<RememberInput, RememberOutput>({
    ...
    call: async (input) => {
      ...
      // 重建 MEMORY.md 索引(扫盘,保证与文件一致)。常驻索引 slot 按会话记忆化
      // (slot.ts),本次新写对本会话的可见性靠结果里的 indexLine(append-only 进历史)。
      const headers = rebuildIndex(sandboxFs, memoryDir);
      const mine = headers.find((h) => h.filePath === target);

      return { data: { path: target, ...(mine ? { indexLine: formatManifest([mine]) } : {}) } };
    },
    mapResult: (output, toolUseId) => ({
      type: 'tool.result',
